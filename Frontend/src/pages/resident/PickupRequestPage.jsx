...
const PickupRequestPage = () => {
  ...
  const selectedAddress = useMemo(
    () =>
      addresses.find(
        (address) => address._id === formData.addressId
      ),
  ...
  const handleCheckCoverage = async () => {
    ...
    try {
      const { data } = await api.get(
        `/residents/addresses/${formData.addressId}/coverage`
      )
      ...
      setCoverageStatus({
        ...
        message:
          coverage?.message ||
          'Coverage information could not be determined.',
        serviceArea:
      ...
      if (coverage?.covered) {
        toast.success(
          'Collection is available for this address'
        )
      } else {
        toast.error(
          'Collection is not available for this address'
        )
      }
    } catch (error) {
      setCoverageStatus({
  ...
  const handleSubmit = async (event) => {
    ...
    // Feature 6 protection:
    // The resident must successfully verify coverage
    // before submitting the request.
    if (coverageStatus?.covered !== true) {
      toast.error(
        'Please check service coverage before submitting the pickup request'
      )
      return
    }
        ...
        >
          <div>
            ...
            <p className="text-sm text-gray-400 mt-1">
              Check service coverage first.
              The system will then automatically
              select an available collector from
              the chosen service area.
            </p>
          </div>
              ...
              <div>
                ...
                {addresses.length === 0 ? (
                  <div className="rounded-xl border border-yellow-800/50 bg-yellow-900/20 p-4 text-sm text-yellow-200">
                    Add an address from your
                    Profile before creating a
                    pickup request.
                  </div>
                ) : (
                  <select
                              ...
                              {coverageStatus.serviceArea && (
                                <p className="text-xs text-gray-500 mt-2">
                                  ...
                                  {coverageStatus
                                    .serviceArea
                                    .city
                                    ? `, ${coverageStatus.serviceArea.city}`
                                    : ''}
                                </p>
                              )}
                            </div>
        ...
        {requests.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-400">
              No waste pickup requests yet.
            </p>

            <p className="text-sm text-gray-600 mt-1">
              Create your first request using
              the button above.
            </p>
          </div>
        ) : (
